...
<div class="scoring-wrapper">
    ...
    <form action="{{ $formAction }}" method="POST" id="scoring-form">
        ...
        @if($errors->any())
            <div style="background:#fee2e2;border-left:4px solid #ef4444;padding:12px 16px;border-radius:10px;margin-bottom:20px;font-size:0.875rem;color:#991b1b;">
        ...
        <div class="criteria-card" style="margin-top:16px;">
            ...
            <textarea name="catatan" rows="3" placeholder="Masukkan catatan tambahan untuk peserta ini..." style="width:100%;padding:10px;border:1.5px solid #e2e8f0;border-radius:10px;font-family:inherit;font-size:0.875rem;resize:vertical;margin-top:8px;">{{ old('catatan', $old?->catatan) }}</textarea>
        </div>
...
.scoring-icon i { width: 20px; height: 20px; }
.scoring-title { font-family: 'Outfit', sans-serif; font-size: 1.05rem; font-weight: 700; margin: 0 0 3px; }
.scoring-subtitle { font-size: 0.8rem; color: #64748b; margin: 0; }
...
.live-label { color: rgba(255,255,255,0.6); font-size: 0.8rem; font-weight: 500; }
.live-score {
...
.live-max { color: rgba(255,255,255,0.4); font-size: 1rem; align-self: flex-end; padding-bottom: 4px; }
...
.criteria-card:focus-within { border-color: #6366f1; }
.criteria-top { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 12px; }
.criteria-info { flex: 1; }
.criteria-label { display: block; font-size: 0.875rem; font-weight: 700; color: #1e293b; margin-bottom: 3px; }
.criteria-desc { font-size: 0.75rem; color: #64748b; line-height: 1.4; }
.criteria-pct {
    flex-shrink: 0;
    background: #eef2ff;
    color: #4f46e5;
...
.criteria-slider::-webkit-slider-thumb:hover { transform: scale(1.2); }
.slider-val {
    font-family: 'Outfit', sans-serif;
    font-size: 1.2rem;
    font-weight: 800;
...
.weighted-display { font-size: 0.72rem; color: #94a3b8; margin-top: 6px; }
.scoring-actions {
    display: flex;
...
@else
<div style="background:#f8fafc;border:2px dashed #e2e8f0;border-radius:16px;padding:32px;text-align:center;margin-top:24px;">
    <p style="color:#94a3b8;font-size:0.9rem;">⚠️ Peserta ini belum memilih kategori nominasi. Form penilaian akan muncul setelah peserta mengisi nominasi.</p>
</div>
@endif
