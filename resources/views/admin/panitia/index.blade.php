...
@if(session('success'))
<div id="panitia-success-alert" class="mb-6 p-4 bg-green-50 border border-green-200 rounded-2xl flex items-center justify-between transition-all duration-500">
    <div class="flex items-center gap-3 text-green-700 font-bold text-sm">
        <i data-feather="check-circle" class="w-5 h-5"></i>
        {{ session('success') }}
    </div>
    <button onclick="document.getElementById('panitia-success-alert').style.display = 'none'" class="text-green-400 hover:text-green-600"><i data-feather="x" class="w-4 h-4"></i></button>
</div>
<script>
    setTimeout(() => {
        const el = document.getElementById('panitia-success-alert');
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-[28px] p-8 shadow-sm border border-slate-100">
            ...
            <form action="{{ route('admin.panitia.store') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Nama Panitia</label>
                    <input type="text" name="name" value="{{ old('name') }}" required
                        class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-all"
                ...
                <button type="submit" class="w-full py-3 bg-gradient-to-r from-blue-600 to-blue-800 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-100 hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
                    <i data-feather="plus" class="w-4 h-4"></i> Buat Akun Panitia
                </button>
            </form>
        </div>
    </div>
        ...
        <div class="bg-white rounded-[28px] shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-8 border-b border-slate-50 flex items-center justify-between">
                <h3 class="text-lg font-black text-slate-800 flex items-center gap-3">
                    ...
                    Daftar Panitia ({{ $panitias->count() }})
                </h3>
            </div>
            @if($panitias->isEmpty())
            <div class="py-20 text-center">
                <i data-feather="user-x" class="w-10 h-10 text-slate-200 mx-auto mb-4"></i>
                <p class="text-slate-400 text-sm font-medium">Belum ada akun panitia. Tambahkan melalui form di kiri.</p>
            </div>
            @else
            <table class="w-full">
                <thead>
                    ...
                    <tr class="border-t border-slate-50 hover:bg-slate-50 transition-colors">
                        <td class="px-8 py-5">
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-700 rounded-xl flex items-center justify-center text-white font-black text-sm">
                                    {{ substr($panitia->name, 0, 1) }}
                                </div>
                                <span class="font-bold text-slate-800 text-sm">{{ $panitia->name }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-5 text-sm text-slate-500">{{ $panitia->email }}</td>
                        <td class="px-8 py-5 text-xs text-slate-400 font-medium">{{ $panitia->created_at->format('d M Y') }}</td>
                        <td class="px-8 py-5">
